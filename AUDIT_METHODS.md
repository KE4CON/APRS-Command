...
**When you copy this file into another repo, replace this one table with that repo's values.** The table
below is filled in for **APRS Command** as a worked example.

| Hook | This repo (APRS Command) |
|---|---|
| **Languages / stack** | C# .NET 10 + Avalonia 11.3.7 (Windows/macOS/Linux/Pi) |
| **Build command** | `dotnet build` (⚠ close the running app first — Windows locks its DLLs) |
| **Test command** | `dotnet test tests/Aprs.Tests/Aprs.Tests.csproj` (~1450 tests) |
| **Coverage command** | `dotnet test --collect:"XPlat Code Coverage"` |
| **Static-analysis / lint** | .NET analyzers on at `latest-recommended` + curated root `.editorconfig` (M0 already done) |
| **Run / launch the app** | `dotnet run --project src/Aprs.Desktop` |
| **Input-parsing surfaces** (fuzz targets) | `AprsParser`, `KissFrameCodec`, `AgwpeFrameCodec`, weather source drivers, `AprsNmeaParser`, `MobileCompanionServer` + `AprsCommand.Api` request handlers, GPX/KML import |
| **Reference oracle** | Dire Wolf `decode_aprs.exe` (`C:\Dev\direwolf\…`); drive it with `tests/Aprs.FuzzHarness --decode` (pipe TNC2 lines, diff fields) |
| **Concurrency hot spots** | transport receive loops (KISS/AGWPE/APRS-IS), the runtime coordinators, `StationDatabase`, `AprsMessageStoreService`, `GeofenceService`, `BeaconScheduler`, GPS write-back |
| **Security-sensitive surfaces** | `MobileCompanionServer` (phone web page), `AprsCommand.Api` (REST + WebSocket), `FileHookService` (file import/export), outbound HTTP integrations (RepeaterBook/Winlink/NWS/tiles), Direwolf process launch |
| **Canonical spec / docs to check against** | APRS Protocol Reference 1.2 (WB4APR); `docs/architecture/APRS_SPEC_CONFORMANCE_PLAN.md`; the user manual; `docs/contributing/BUG_HUNTING_PLAYBOOK.md` (project-tailored companion to this file) |
| **Known-latent list** | `docs/contributing/AUDIT_2026-08-10.md` + `AUDIT_2026-08-10-DEEP.md` (tracked residuals); Decisions Log **P2** |
